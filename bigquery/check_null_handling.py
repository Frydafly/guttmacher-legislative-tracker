...
print("🔍 CHECKING NULL HANDLING: looker_comprehensive_bills vs all_historical_bills_unified")
print("=" * 90)
...
WITH comprehensive_nulls AS (
  SELECT 
    'looker_comprehensive_bills' as table_name,
    data_year,
...
for row in results:
    table_short = row.table_name.replace('_historical_bills', '').replace('looker_comprehensive_bills', 'comprehensive')
...
for case_name, null_condition, year_condition in problem_cases:
    ...
    for table in ["all_historical_bills_unified", "looker_comprehensive_bills"]:
        check_query = f"""
        SELECT 
          '{table}' as table_name,
          COUNT(*) as total_bills,
        ...
        FROM `guttmacher-legislative-tracker.legislative_tracker_historical.{table}`
...
# Check if looker_comprehensive_bills is filtering out records or converting NULLs
diagnosis_query = """
SELECT 
  'Record Count Check' as check_type,
  (SELECT COUNT(*) FROM `guttmacher-legislative-tracker.legislative_tracker_historical.all_historical_bills_unified`) as unified_count,
  (SELECT COUNT(*) FROM `guttmacher-legislative-tracker.legislative_tracker_historical.looker_comprehensive_bills`) as comprehensive_count
"""
...
for row in result:
    if row.unified_count != row.comprehensive_count:
        ...
        print(f"   looker_comprehensive_bills:   {row.comprehensive_count:,} bills")
...
print("-" * 50)
print("""
If looker_comprehensive_bills is converting NULLs to FALSE or filtering records:
...
⚠️  AVOID: looker_comprehensive_bills if it's masking data reality
